...
function sanitize_email( $text ) { return strtolower( trim( (string) $text ) ); }
...
function add_action( $hook, $callback ) { $GLOBALS['__cookiedk_hooks']['actions'][ $hook ][] = $callback; }
function add_filter( $hook, $callback ) { $GLOBALS['__cookiedk_hooks']['filters'][ $hook ][] = $callback; }
function apply_filters( $hook, $value ) { return $value; }
function is_admin() { return false; }
function wp_create_nonce( $action ) { return 'nonce-' . $action; }
function check_ajax_referer( $action, $field = 'nonce' ) { return true; }
function wp_send_json_success( $data = array() ) { throw new RuntimeException( 'json_success:' . json_encode( $data ) ); }
function wp_send_json_error( $data = array() ) { throw new RuntimeException( 'json_error:' . json_encode( $data ) ); }
function status_header( $status ) { return $status; }
...
function add_settings_error( $setting, $code, $message, $type ) { return true; }
...
function get_site_url() { return 'https://example.org'; }
function get_bloginfo( $show ) { return 'Eksempel Site'; }
function wp_kses_post( $content ) { return $content; }
function wp_add_privacy_policy_content( $plugin_name, $content ) { return true; }
function set_transient( $key, $value, $expiration ) { $GLOBALS['__cookiedk_transients'][ $key ] = $value; return true; }
function get_transient( $key ) { return isset( $GLOBALS['__cookiedk_transients'][ $key ] ) ? $GLOBALS['__cookiedk_transients'][ $key ] : false; }
function get_option( $name, $default = false ) { return isset( $GLOBALS['__cookiedk_options'][ $name ] ) ? $GLOBALS['__cookiedk_options'][ $name ] : $default; }
function update_option( $name, $value ) { $GLOBALS['__cookiedk_options'][ $name ] = $value; return true; }
class FakeWpdb {
    public $prefix = 'wp_';
    public $insert_id = 0;
    ...
    public function get_charset_collate() { return 'CHARSET utf8mb4'; }
    ...
    public function delete( $table, $where ) {
        ...
        if ( isset( $where['user_fingerprint'] ) ) {
            ...
            $this->consents = array_values( array_filter( $this->consents, function( $row ) use ( $where ) {
